...
var ol_interaction_UndoRedo = class olinteractionUndoRedo extends ol_interaction_Interaction {
  ...
  setMap(map) {
    ...
    if (map) {
      this._mapListener.push(
        map.on("undoblockstart", this.blockStart.bind(this))
      );
  ...
  _watchSources() {
    ...
    if (map) {
      ...
      vectors.forEach(
        function (l) {
          ...
          this._sourceListener.push(
            s.on(["addfeature", "removefeature"], this._onAddRemove.bind(this))
          );
          this._sourceListener.push(
            s.on(
              "clearstart",
      ...
      this._sourceListener.push(
        map.getLayers().on(["add", "remove"], this._watchSources.bind(this))
      );
    }
  }
  ...
  _watchInteractions() {
    ...
    if (map) {
      ...
      this._interactionListener.push(
        map
          .getInteractions()
          .on(["add", "remove"], this._watchInteractions.bind(this))
      );
    }
  }
  ...
  _handleDo(e, undo) {
    ...
    if (e.custom) {
      ...
      switch (e.type) {
        ...
        case "changegeometry": {
          ...
          Object.keys(geom.listeners_).forEach((key) => {
            ...
            geom.listeners_[key].forEach((listener) => {
              if (
                !(key in featureGeometry.listeners_) ||
        ...
        default: {
          console.warn(
            '[UndoRedoInteraction]: "' + e.type + '" is not defined.'
          );
        }
      }
    }
 ...
 * @private
 */
ol_interaction_UndoRedo.prototype._onInteraction.setattributestart = function (
  e
) {
...
ol_interaction_UndoRedo.prototype._onInteraction.rotatestart =
  ...
  ol_interaction_UndoRedo.prototype._onInteraction.modifystart =
    function (e) {
      /**
